...
public class ImportItemsSmdHandler : IRequestHandler<ImportItemsSmdRequest, ItemsResponse>
{
    private readonly ApplicationDbContext _db;
    private readonly BlobService _service;

    public ImportItemsSmdHandler(ApplicationDbContext db, BlobService service)
    {
        _db = db;
        _service = service;
    }

    public async Task<ItemsResponse> Handle(ImportItemsSmdRequest request, CancellationToken cancellationToken)
    {
        var rowsInserted = 0;
        var rowsUpdated = 0;
        foreach (var blobName in request.BlobNames)
        {
            ...
            for (int col = 1; col <= lastCol; col++)
            {
                ...
                {
                    value = ws.Cell(headerRow - 1, col).GetString().Trim();
                }
            ...
            while (r <= lastRow)
            {
                ...
                if (!cell.IsEmpty())
                {
                    importedItems.Add(new ItemDTO
                    {
                        ...
                        Brand = ws.Cell(r, headers["Merek"]).GetString(),
                        Type = ws.Cell(r, headers["Jenis"]).GetString(),
            ...
            var existingItems = await _db.Items
                .Where(q => codes.Contains(q.Code) && q.WarehouseId == WarehouseEnums.Samarinda)
            ...
            var newItems = itemsToInsert
                .Select(item => new Item
                {
                    ...
                    Code = item.Code,
                    WarehouseId = WarehouseEnums.Samarinda
                }).ToList();
